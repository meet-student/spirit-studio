...
export default function Datasets() {
  ...
  const headerCreateAction = (
    <HeaderCreateAction href="/datasets/new" tooltip={t('actions.createDataset')}>
  ...
  if (error && is401UnauthorizedError(error)) {
    return (
      <PageLayout breadcrumbs={<PageBreadcrumbs crumbs={crumbs} />}>
        <h1 className="sr-only">{t('table.datasets')}</h1>
  ...
  if (error) {
    return (
      <PageLayout breadcrumbs={<PageBreadcrumbs crumbs={crumbs} />}>
        <h1 className="sr-only">{t('table.datasets')}</h1>
        <EmptyState tone="error" variant="fill" titleSlot={t('errors.failedDatasets')} descriptionSlot={error.message} />
      </PageLayout>
    );
  }
