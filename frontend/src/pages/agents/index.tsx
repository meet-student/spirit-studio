...
function Agents() {
  ...
  if (error && is401UnauthorizedError(error)) {
    return (
      <PageLayout breadcrumbs={<PageBreadcrumbs crumbs={crumbs} />}>
        <h1 className="sr-only">{t('table.agents')}</h1>
  ...
  if (error) {
    return (
      <PageLayout breadcrumbs={<PageBreadcrumbs crumbs={crumbs} />}>
        <h1 className="sr-only">{t('table.agents')}</h1>
        <EmptyState tone="error" variant="fill" titleSlot={t('errors.failedAgents')} descriptionSlot={error.message} />
      </PageLayout>
    );
  }
