...
export default function Experiments() {
  ...
  const experimentDatasetOptions = useMemo(
    () => getExperimentDatasetOptions(datasets, t('filters.allDatasets')),
    [datasets, t],
  );
  ...
  if (error && is401UnauthorizedError(error)) {
    return (
      <PageLayout breadcrumbs={<PageBreadcrumbs crumbs={crumbs} />}>
        <h1 className="sr-only">{t('table.experiments')}</h1>
    ...
    return (
      <PageLayout breadcrumbs={<PageBreadcrumbs crumbs={crumbs} />}>
        ...
        <EmptyState
          tone="error"
          variant="fill"
          titleSlot={t('errors.failedExperiments')}
