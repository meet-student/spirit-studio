...
function toSidebarLink(item: NavItem, translate: (key: string) => string): SidebarNewLink {
  const { Icon } = item;
  return { name: item.translationKey ? translate(item.translationKey) : item.name, url: item.url, icon: <Icon /> };
}
...
function MoreRow({ onClick }: { onClick: () => void }) {
  ...
  return (
    <SidebarNew.NavLink
      state={state}
      link={{ name: t('more'), url: '#', icon: <Ellipsis /> }}
      render={
        <button type="button" onClick={onClick}>
          <Ellipsis />
          <SidebarNew.NavLabel state={state}>{t('more')}</SidebarNew.NavLabel>
        </button>
      }
    />
  );
...
export function AppSidebar() {
  ...
  return (
    <SidebarNew aria-label={t('common:sidebar', { defaultValue: 'Sidebar' })}>
      <SidebarNew.CommandHeader>
        <SidebarNew.Brand logo={<LogoWithoutText className="size-6" />} title={t('common:appName')} />
        {isUserAuthenticated && <AuthStatus />}
        {!isMobile && (
          <SidebarNew.SearchTrigger
            aria-label={t('common:searchAndNavigate')}
