...
export const StudioConfigForm = ({ initialConfig, onSave, variant = 'default' }: StudioConfigFormProps) => {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    onSave?.();
    toast.success(t('actions.configurationSaved', { defaultValue: 'Configuration saved' }));
  };
  ...
  const connectionFields = [
    {
      name: 'url',
      label: t('connection.instanceUrl'),
      placeholder: t('connection.instanceUrlPlaceholder'),
      required: true,
      defaultValue: initialConfig?.baseUrl,
    },
    {
      name: 'apiPrefix',
      label: t('connection.apiPrefix'),
      placeholder: t('connection.apiPrefixPlaceholder'),
    ...
    <TooltipProvider delayDuration={0}>
      <form onSubmit={handleSubmit} className={isFactoryLayout ? 'flex flex-col gap-4' : 'space-y-6'}>
        <FieldsContainer>
          ...
          {isFactoryLayout ? (
            <SettingsRow label={t('headers.title')}>
