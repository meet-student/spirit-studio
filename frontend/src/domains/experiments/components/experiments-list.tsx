    ...
    <EntityList.RowWrapper {...rowProps} onSelectRow={onSelect ?? (() => linkRef.current?.click())}>
      ...
      <EntityList.ActionsCell className="pl-2">
        <Button
          ...
          variant="ghost"
          size="icon-sm"
          tooltip={t('tooltips.deleteExperiment')}
          aria-label={`${t('tooltips.deleteExperiment')} ${exp.name ?? exp.id}`}
          onClick={(e: MouseEvent) => {
            e.stopPropagation();
            onDelete();
          }}
...
export function ExperimentsList({
  ...
  const columnHeaders: { label: string; className?: string; sortKey?: ExperimentsSortKey }[] = [
    { label: t('table.experiment') },
    { label: t('table.description') },
    { label: t('table.dataset') },
    { label: t('table.target') },
    { label: t('table.status'), sortKey: 'status' },
    { label: t('table.items'), className: 'text-center' },
    { label: t('table.processed'), className: 'text-center' },
    { label: t('table.errored'), className: 'text-center' },
    { label: t('table.review'), className: 'text-center' },
    { label: t('table.date'), sortKey: 'createdAt' },
  ];
      ...
      {filteredData.map((exp, index) => {
        ...
        return (
          <EntityList.RowWrapper key={exp.id} {...getRowProps(index)} onSelectRow={toggle}>
            <EntityList.SelectCell
              checked={isSelected}
              onToggle={toggle}
              aria-label={t('tooltips.selectExperiment', { id: exp.id })}
            />
